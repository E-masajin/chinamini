...
- **未来予測クイズ**: 記入式で未来を予測するクイズモード
- **AI機能**:
  - **AI柔軟判定**: OpenAI APIで記入式回答を柔軟に評価（表記揺れ許容）
  - **AI問題生成**: テーマを入力するだけで予測問題を自動生成
- **ゲーミフィケーション**:
  - ポイントシステム（正解+10pt、全問正解ボーナス+50pt）
  - 11種類のバッジ（予測マスター、連続正解王など）
  - 総合/週間/月間ランキング
...
- ✅ ID紐付け問題振り分けロジック
- ✅ 回答時間計測機能
- ✅ 参加期限・答え発表時刻のタイムライン表示

### AI機能 🤖
- ✅ **AI柔軟判定**: 記入式回答を柔軟に評価
  - 表記揺れ許容（例: 「カレー」「カレーライス」「チキンカレー」）
  - 時刻の表記揺れ許容（例: 「12:30」「12時30分」「午後0時半」）
  - 数値の範囲判定（±5%以内なら正解）
  - 単位の有無を無視（例: 「800」「800円」）
- ✅ **AI問題生成**: テーマから予測問題を自動生成
  - 例: 「オフィスでの身近な予測」→ 3問自動生成
  - 記入式問題に対応
  - 参加期限・答え発表時刻を自動設定
...
| POST | `/admin/api/prediction/questions/:questionId/verify` | 予測問題の答え合わせ |

### AI API 🤖

| メソッド | パス | 説明 |
|---------|------|------|
| POST | `/admin/api/ai/verify-prediction` | AI柔軟判定（記入式回答を柔軟に評価） |
| POST | `/admin/api/ai/generate-prediction-questions` | AI問題生成（テーマから予測問題を自動生成） |
| POST | `/admin/api/ai/generate-questions` | AI問題生成（ファイルから4択問題を自動生成） |

### ゲーミフィケーションAPI 🎮

| メソッド | パス | 説明 |
|---------|------|------|
| GET | `/api/gamification/users/:userId/points` | ユーザーポイント取得 |
| GET | `/api/gamification/users/:userId/badges` | ユーザーバッジ取得 |
| GET | `/api/gamification/users/:userId/statistics` | ユーザー統計取得 |
| GET | `/api/gamification/leaderboard/:type` | ランキング取得（total/weekly/monthly） |
...
### セキュリティ ✅ 強化済み
- ✅ 管理者パスワードのハッシュ化（SHA-256 + Salt）
- ✅ セッション管理実装済み（24時間有効期限）
- ✅ CSRFトークン対策実装済み
- ✅ アカウントロック機能（5回失敗で15分ロック）
- ✅ 監査ログ機能（ログイン/ログアウト/失敗記録）
- ✅ 入力検証とサニタイズ処理
- ✅ タイミング攻撃対策
- ⚠️ レガシーAPI（/admin/api/login）は後方互換性のため残存
- 📌 新規実装は /admin/api/v2/* エンドポイントを使用推奨

---

**開発者**: Claude Code  
**最終更新**: 2026-01-14  
**バージョン**: 2.1 (セキュリティ強化 + アーキテクチャ改善)
